...
function ItkVtkViewProxy(publicAPI, model) {
  ...
  function setVisualizationMode(axisIndex) {
    ...
    if (axisIndex === -1) {
      ...
      if (model.volumeCameraState) {
        ...
        model.camera.setPhysicalTranslation(...model.volumeCameraState.physicalTranslation);
      }
  ...
  function updateAnnotations(callData) {
    ...
    model.annotationPicker.pick([renderPosition.x, renderPosition.y, 0.0], callData.pokedRenderer);
    ...
    if (model.volumeRepresentation) {
      ...
      if (ijk.length > 0 && worldPositions.length > 0) {
        ...
        model.lastPickedValues = {
          ...
          value: model.labelIndex === null ? fusedValue : fusedValue.slice(0, model.labelIndex),
          label: model.labelIndex === null ? null : fusedValue[model.labelIndex],
          annotation,
  ...
  publicAPI.renderLater = () => {
    ...
    if (!model.widgetManagerInitialized) {
      ...
      model.axesOriginWidget = model.widgetManager.addWidget(model.axesOriginLabel);
      ...
      const color = model.axesGridActor.getProperty().getColor()[0] === 0.0 ? 'black' : 'white';
      ...
      model.axesXActor = model.axesXWidget.getRepresentations()[1].getActors()[0];
      ...
      model.axesYActor = model.axesYWidget.getRepresentations()[1].getActors()[0];
      ...
      model.axesZActor = model.axesZWidget.getRepresentations()[1].getActors()[0];
