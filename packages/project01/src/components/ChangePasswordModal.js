    ...
    return (
      ...
      >
        <Form labelCol={{ span: 6 }} wrapperCol={{ span: 17, offset: 1 }} onSubmit={this.handlePasswordSubmit}>
          <Form.Item label="用户密码" hasFeedback>
            {getFieldDecorator('password', {
              rules: [
                ...
                { type: 'string', min: 6, message: '密码至少需要6位' },
                {
                  validator: this.validateToNextPassword
                },
                {
                  pattern: /[a-zA-Z]+/,
                  message: '密码需要包含至少一位字母'
                }
              ]
            })(
              <Input.Password
          ...
          <Form.Item label="再次确认" hasFeedback>
            {getFieldDecorator('confirm', {
              rules: [
                { required: true, message: '请再次填写用户密码！' },
                { type: 'string', min: 6, message: '密码至少需要6位' },
                {
...
export default connect()(Form.create()(ChangePasswordModal))
